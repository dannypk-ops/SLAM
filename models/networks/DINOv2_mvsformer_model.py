...
class DINOv2MVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values, tmp=[5., 5., 5., 1.]):
        ...
        if not self.training:
            ...
            #                              align_corners=Align_Corners_Range)
            vit_feat = vit_feat.to(torch.float32)  # Convert to float32 before interpolation
            vit_feat = F.interpolate(vit_feat, size=(conv31_h, conv31_w), mode='bilinear', align_corners=False)
