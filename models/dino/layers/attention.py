...
def scaled_dot_product_attention(q, k, v, scale):
    # q, k, v are [B, N, L, C] where:
    # B: Batch size
    # N: Number of heads
    # L: Sequence length (1729)
    # C: Feature dimension per head (64)
    
    # Step 1: Calculate scaled dot-product attention scores
    attn_logits = torch.matmul(q, k.transpose(-2, -1))  # Shape: [B, N, L, L]
    
    # Apply scaling factor
    attn_logits = attn_logits * scale  # Apply softmax scale (e.g., 0.125)
    
    # Step 2: Apply softmax to attention logits along the last dimension (the key dimension)
    attn_weights = F.softmax(attn_logits, dim=-1)  # Shape: [B, N, L, L]
    
    # Step 3: Apply attention weights to the values
    attn_output = torch.matmul(attn_weights, v)  # Shape: [B, N, L, C]
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, return_attn=False) -> Tensor:
        ...
        x, _ = scaled_dot_product_attention(q, k, v, scale=softmax_scale)
