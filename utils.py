...
def load_model(config):
    ...
    # temp setting
    # if hasattr(mvsnet, 'vit_args') and 'height' in mvsnet.vit_args and 'width' in mvsnet.vit_args:
    #     mvsnet.vit_args['height'] = args.max_h // 2
    #     mvsnet.vit_args['width'] = args.max_w // 2
...
def model_loader(args, tstamps, poses, ref_disp):
    ...
    proj_matrices = torch.stack((poses, intr_matrices), dim=1)
    ...
    proj_matrices_ms = {
            "stage1": stage0_pjmats.unsqueeze(0),
            "stage2": stage1_pjmats.unsqueeze(0),
            "stage3": stage2_pjmats.unsqueeze(0),
            "stage4": stage3_pjmats.unsqueeze(0),
        }
    
    ref_depth = 1 / ref_disp
    val_depths = ref_depth[(ref_depth > 0.001) & (ref_depth < 100)]
    ...
    return images.unsqueeze(0).cuda(), proj_matrices_ms, depth_values.cuda()